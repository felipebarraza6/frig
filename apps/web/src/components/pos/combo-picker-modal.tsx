...
import { X, Boxes, Search } from "lucide-react";
...
export function ComboPickerModal({ combos, onSelect, onClose }: ComboPickerModalProps) {
  ...
  return (
    ...
    >
      <div className="flex max-h-[85vh] w-full max-w-2xl flex-col rounded-2xl border border-border/60 bg-card shadow-xl">
        <div className="flex shrink-0 items-center justify-between border-b border-border/60 px-4 py-3">
          <div className="flex items-center gap-2">
            <Boxes className="h-5 w-5 text-primary" />
            <h2 className="text-base font-semibold">Combos</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
