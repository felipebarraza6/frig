...
export default function ProfilePage() {
  ...
  const { data: branchContract } = useQuery({
    queryKey: ["branch-contract", currentBranchId],
    ...
    <div className="mx-auto flex min-h-full w-full max-w-2xl flex-col gap-6 p-4 sm:p-6">
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          ...
          {activeSection === "personal" && !loadingProfile && (
            <div className="mt-10 border-t border-border pt-6">
              ...
              <dl className="grid gap-x-4 gap-y-4 sm:grid-cols-2">
                ...
                {branchContract?.plan_name && (
                  <div className="flex items-start gap-2.5">
                    <Store className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
                    <div>
                      <dt className="text-xs text-muted-foreground">Plan de la sucursal</dt>
                      <dd className="mt-0.5 text-sm font-medium">
                        {branchContract.plan_name}
                        {branchContract.business_name ? ` · ${branchContract.business_name}` : ""}
                      </dd>
                    </div>
                  </div>
                )}
                {branchContract?.plan_expiration_date && (
                  <div className="flex items-start gap-2.5">
                    <CalendarDays className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
                    <div>
                      <dt className="text-xs text-muted-foreground">Vencimiento del plan</dt>
                      <dd className="mt-0.5 text-sm font-medium">
                        {formatDate(branchContract.plan_expiration_date)}
                      </dd>
                    </div>
                  </div>
                )}
