...
export function ProfileSubscriptionTab() {
  ...
  const isExpired = branch?.plan_expiration_date && new Date(branch.plan_expiration_date) < new Date();
      ...
      {loadingBranch ? (
        ...
        <div className="flex flex-col gap-4 rounded-xl border border-border bg-card p-5 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            ...
            >
              {!branch.plan ? "Inactivo" : isExpired ? "Expirado" : "Activo"}
            </span>
          </div>
          ...
          {isOwnerOrAdmin && (
            <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-border pt-4">
              <Button onClick={() => setIsPlanDialogOpen(true)} size="sm" variant={isExpired ? "default" : "outline"}>
